...
print("Task 4: It's so boring Task! I would't to make ")
print("_" * 105)
print('\n')
...
def get_jokes_v2(Number):
    ...
    random.sample(список, количество элементов) - выбор значений без повторений

     for elem in range ( Number ): цикл собирания из списков
        elems_list = f'{elems_list1[elem]} {elems_list2[elem]} {elems_list3[elem]}'
        joke_list.append ( elems_list ) - добавление с писок полученных значений

    joke_list.append(elems_list) - добавляются выражения в список

    print ( joke_list ) - вывод шуток из списка
    """
    ...
    adjectives = ["веселый", "яркий", "зеленый", "утопичный", "мягкий"]
    joke_list = []
    elems_list1 = random.sample ( nouns, Number )
    elems_list2 = random.sample ( adverbs, Number )
    elems_list3 = random.sample ( adjectives, Number )
    for elem in range ( Number ):
        elems_list = f'{elems_list1[elem]} {elems_list2[elem]} {elems_list3[elem]}'
        joke_list.append ( elems_list )
    print ( joke_list )
